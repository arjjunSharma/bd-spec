...
3.  **Deploy/Host:** Push the project to a GitHub repository's `main` branch. The included GitHub Actions workflow deploys it automatically.
4.  **Enable Pages once:** In the repository, open **Settings → Pages**, set **Source** to **GitHub Actions**, and wait for the workflow to finish.
5.  **Share the Link:** Your page will be available at `https://YOUR-USERNAME.github.io/YOUR-REPOSITORY/`.

### Open the local page on a mobile phone

To test the page from a phone while developing:

1. Connect the phone and computer to the same Wi-Fi network.
2. In this folder, run `npm install`, then `npm start`.
3. Find the **External** URL printed by BrowserSync, such as `http://192.168.1.10:7777`.
4. Open that complete URL on the phone. Do not use `localhost`, `127.0.0.1`, or the computer's `file://` URL.

If the URL still does not load, temporarily allow incoming connections for Node.js in macOS Firewall and make sure the phone is not using a guest Wi-Fi network or mobile data. BrowserSync listens on the computer's network interfaces so other devices on the same local network can connect.
